...
from app.application.exceptions import (
    CodeTaskAlreadyUsedError, CodeTaskNotFoundError
)
...
class UpdateCodeTaskUseCase:
    ...
    async def execute(self, command: UpdateCodeTaskCommand) -> CodeTask:
        async with self.uow:
            ...
            await self.course_access_service.ensure_can_manage_section(  # Noqa E501
                actor=command.actor,
                section_id=code_task.section_id,
            )

            has_submissions = await (
            ...
            if execution_changed:
                try:
                    code_task.ensure_execution_policy_can_be_changed(
                        has_submissions
                    )
            ...
            if learning_changed:
                try:
                    code_task.ensure_learning_policy_can_be_changed(
                        has_submissions
                    )
                except InvalidCodeTaskError as exc:
                    raise CodeTaskAlreadyUsedError(str(exc)) from exc
