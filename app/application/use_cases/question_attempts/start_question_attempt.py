        ...
        async with self.uow:
            ...
            return StartQuestionAttemptDTO(
                ...
                selected_option_ids=list(
                    last_attempt.selected_option_ids
                ) if last_attempt else [],
                last_result_status=last_attempt.result_status if (
                    last_attempt
                ) else None,
                last_awarded_points=last_attempt.awarded_points if (
                    last_attempt
                ) else None,
                answer_options=[
                    QuestionAttemptAnswerOptionDTO(
                    ...
                    for option in sorted(
                        answer_options, key=lambda item: item.position
                    )
                ],
            )
