...
from app.application.exceptions import (
    PermissionDeniedError, QuestionNotFoundError, SectionNotFoundError
)
...
class SubmitQuestionAnswerUseCase:
    ...
    async def execute(
        self, command: SubmitQuestionAnswerCommand
    ) -> QuestionAttempt:
        if not command.actor.can_take_learning_activities():
            raise PermissionDeniedError(
                'User cannot submit question answers.'
            )
        ...
        async with self.uow:
            ...
            attempts = await (
                self.uow.question_attempts.get_by_student_and_question(
            ...
            if attempt.is_correct():
                section = await self.uow.sections.get_by_id(
                    question.section_id
                )
                if section is None:
