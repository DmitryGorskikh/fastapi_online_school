...
class GetQuestionAttemptResultUseCase:
    ...
    async def execute(
        ...
        async with self.uow:
            attempt = await self.uow.question_attempts.get_by_id(
                command.attempt_id
            )
            if attempt is None:
                raise QuestionAttemptNotFoundError(
                    'Question attempt not found.'
                )
            if attempt.student_id != command.actor.id:
                await self.course_access_service.ensure_can_view_question_results(  # noqa: E501
                    actor=command.actor,
