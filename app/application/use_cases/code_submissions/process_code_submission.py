...
from app.application.exceptions import (
    CodeSubmissionNotFoundError, CodeTaskNotFoundError
)
from app.application.interfaces.code_execution_gateway import (
    CodeExecutionGateway
)
from app.application.interfaces.unit_of_work import UnitOfWork
from app.application.use_cases.code_submissions.complete_code_submission import (  # Noqa E501
    ...
    async def execute(self, command: ProcessCodeSubmissionCommand) -> None:
        async with self.uow:
            ...
            code_task = await self.uow.code_tasks.get_by_id(
                submission.code_task_id
            )
            if code_task is None:
                raise CodeTaskNotFoundError('CodeTask not found.')

            test_cases = await self.uow.test_cases.list_by_code_task_id(
                code_task.id
            )
        ...
        result = await self.execution_gateway.execute(
            code_task, submission, test_cases
        )
