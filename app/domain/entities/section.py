...
class Section:
    ...
    def can_be_completed(self) -> bool:
        return bool(
            self.question_ids or self.task_ids or self.code_task_ids
        )
    ...
    def is_completed_by(
        ...
        return (
            all(question_id in completed_question_ids for question_id in self.question_ids)  # noqa: E501
            and all(task_id in completed_task_ids for task_id in self.task_ids)
            and all(code_task_id in completed_code_task_ids for code_task_id in self.code_task_ids)  # noqa: E501
        )
