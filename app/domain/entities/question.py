...
class Question:
    ...
    def _validate(self) -> None:
        ...
        if self.max_attempts < 1:
            raise InvalidQuestionError(
                'Question max_attempts must be positive.'
            )
        if self.reward_points < 1:
            raise InvalidQuestionError(
                'Question reward_points must be positive.'
            )
    ...
    def validate_answer_options_configuration(
        ...
        correct_options_count = sum(
            1 for option in answer_options if option.is_correct
        )
        ...
        if self.is_single_choice() and correct_options_count != 1:
            raise InvalidQuestionError(
                'Single choice question must have exactly '
                'one correct answer option.'
            )
        if self.is_multiple_choice() and correct_options_count < 2:
            raise InvalidQuestionError(
                'Multiple choice question must have at least '
                'two correct answer options.'
            )
    ...
    def is_correct_selection(
        ...
        if not selected_ids.issubset(actual_option_ids):
            raise InvalidQuestionResultError(
                'Selected options contain unknown ids.'
            )
        correct_option_ids = {
