...
class CodeTask:
    ...
    def _validate(self) -> None:
        if not self.title or not self.title.strip():
            raise InvalidCodeTaskError(
                'CodeTask title cannot be empty.'
            )
        if not self.statement or not self.statement.strip():
            raise InvalidCodeTaskError(
                'CodeTask statement cannot be empty.'
            )
        if self.position < 1:
            raise InvalidCodeTaskError(
                'CodeTask position must be positive.'
            )
        if self.max_attempts < 1:
            raise InvalidCodeTaskError(
                'CodeTask max_attempts must be positive.'
            )
        if self.reward_points < 1:
            raise InvalidCodeTaskError(
                'CodeTask reward_points must be positive.'
            )
        if self.time_limit_seconds < 1:
            raise InvalidCodeTaskError(
                'CodeTask time_limit_seconds must be positive.'
            )
        if self.memory_limit_mb < 16:
            raise InvalidCodeTaskError(
                'CodeTask memory_limit_mb is too small.'
            )
    ...
    def next_submission_number(
        self, existing_submissions_count: int
    ) -> int:
        if existing_submissions_count < 0:
            raise InvalidCodeTaskError(
                'Existing submissions count cannot be negative.'
            )
    ...
    def create_submission(
        ...
        return CodeSubmission(
            ...
            attempt_number=self.next_submission_number(
    ...
    def ensure_execution_policy_can_be_changed(
        self, has_submissions: bool
    ) -> None:
        if has_submissions:
            raise InvalidCodeTaskError(
                'CodeTask execution policy cannot '
                'be changed after submissions.'
            )

    def ensure_learning_policy_can_be_changed(
        self, has_submissions: bool
    ) -> None:
        if has_submissions:
            raise InvalidCodeTaskError(
    ...
    def ensure_test_cases_can_be_changed(
        self, has_submissions: bool
    ) -> None:
        if has_submissions:
            raise InvalidCodeTaskError(
