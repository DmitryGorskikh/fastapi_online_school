    ...
    def _validate(self) -> None:
        ...
        if self.passed_test_cases > self.total_test_cases:
            raise InvalidExecutionResultError(
                ...
                'cannot exceed total_test_cases.'
            )
        if self.status is ExecutionStatus.PASSED and (
        ...
        if self.status is ExecutionStatus.PASSED and (
            self.passed_test_cases != self.total_test_cases
        ):
            raise InvalidExecutionResultError(
                'Passed execution result must pass all test cases.'
            )
        if self.status is ExecutionStatus.ERROR and not (
