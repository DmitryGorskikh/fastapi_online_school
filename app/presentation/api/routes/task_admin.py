...
from app.application.use_cases.code_tasks.delete_code_task import (
    DeleteCodeTaskCommand, DeleteCodeTaskUseCase
)
...
from app.application.use_cases.tasks.delete_task import (
    DeleteTaskCommand, DeleteTaskUseCase
)
...
from app.application.use_cases.test_cases.delete_test_case import (
    DeleteTestCaseCommand, DeleteTestCaseUseCase
)
...
from app.presentation.api.dependencies import (
    ...
    get_remove_code_task_use_case,
    get_remove_test_case_use_case,
...
async def remove_task(
    task_id: UUID,
    actor: User = Depends(get_current_author_or_admin),
    use_case: DeleteTaskUseCase = Depends(
        get_remove_task_use_case
    ),
):
    await use_case.execute(
        DeleteTaskCommand(actor=actor, task_id=task_id)
    )
    logger.info(
        'Task deleted',
        extra={
...
@router.delete(
    ...
    status_code=status.HTTP_204_NO_CONTENT,
    description=(
            "Deletes an existing code task by its identifier. "
    ),
    responses={
        400: {
            "description": "Domain or application validation error.",
...
async def remove_code_task(
    code_task_id: UUID,
    actor: User = Depends(get_current_author_or_admin),
    use_case: DeleteCodeTaskUseCase = Depends(
        get_remove_code_task_use_case
    ),
):
    await use_case.execute(
        DeleteCodeTaskCommand(actor=actor, code_task_id=code_task_id)
    )
    logger.info(
        'Code task deleted',
        extra={
            'event': 'code_task_deleted',
            'code_task_id': str(code_task_id),
            'actor_id': str(actor.id),
        },
    )
...
@router.delete(
    ...
    responses={
        ...
        404: {
            "description": "TestCase was not found.",
            "model": ErrorResponse,
        },
    },
)
async def remove_test_case(
    test_case_id: UUID,
    actor: User = Depends(get_current_author_or_admin),
    use_case: DeleteTestCaseUseCase = Depends(get_remove_test_case_use_case),
):
    await use_case.execute(
