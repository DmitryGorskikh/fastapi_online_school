...
class SqlAlchemyCodeTaskRepository(CodeTaskRepository):
    ...
    async def get_by_ids(self, code_task_ids: list[UUID]) -> list[CodeTask]:
        ...
        stmt = select(CodeTaskModel).where(
            CodeTaskModel.id.in_(
                [str(code_task_id) for code_task_id in code_task_ids]
            )
        )
    ...
    async def remove(self, code_task: CodeTask) -> None:
        model = await self.session.get(CodeTask, str(code_task.id))
        if model is None:
            return
