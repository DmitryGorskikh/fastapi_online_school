...
from app.infrastructure.database.mappers.answer_option_mapper import (
    AnswerOptionMapper
)
from app.infrastructure.database.models.answer_option_model import (
    AnswerOptionModel
)
...
class SqlAlchemyAnswerOptionRepository(AnswerOptionRepository):
    ...
    async def get_by_ids(
        self, answer_option_ids: list[UUID]
    ) -> list[AnswerOption]:
    ...
    async def update(self, answer_option: AnswerOption) -> None:
        model = await self.session.get(
            AnswerOptionModel, str(answer_option.id)
        )
        if model is None:
            return
