...
class SqlAlchemyTestCaseRepository(TestCaseRepository):
    ...
    async def get_by_ids(self, test_case_ids: list[UUID]) -> list[TestCase]:
        if not test_case_ids:
            return []
        stmt = (
            select(TestCaseModel)
            .where(TestCaseModel.id.in_([str(item) for item in test_case_ids]))
        )
        result = await self.session.execute(stmt)
        return [
            TestCaseMapper.to_domain(model) for model in result.scalars().all()
        ]
    ...
    async def remove(self, test_case: TestCase) -> None:
        model = await self.session.get(TestCase, str(test_case.id))
        if model is None:
            return
