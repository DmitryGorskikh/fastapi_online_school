...
from app.application.interfaces.repositories.question_attempt_repository import (  # noqa: E501
...
class SqlAlchemyQuestionAttemptRepository(QuestionAttemptRepository):
    ...
    async def get_by_id(self, attempt_id: UUID) -> QuestionAttempt | None:
        stmt = select(
            QuestionAttemptModel
        ).where(QuestionAttemptModel.id == str(attempt_id))
        result = await self.session.execute(stmt)
        model = result.scalar_one_or_none()
        return None if model is None else QuestionAttemptMapper.to_domain(
            model
        )
    ...
    async def get_by_student_and_question(
        ...
        result = await self.session.execute(stmt)
        return [QuestionAttemptMapper.to_domain(
            model
