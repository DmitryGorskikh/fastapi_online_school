...
from app.infrastructure.database.mappers.task_attempt_mapper import (
    TaskAttemptMapper
)
from app.infrastructure.database.models.task_attempt_model import (
    TaskAttemptModel
)
...
class SqlAlchemyTaskAttemptRepository(TaskAttemptRepository):
    ...
    async def get_by_id(self, attempt_id: UUID) -> TaskAttempt | None:
        stmt = select(TaskAttemptModel).where(
            TaskAttemptModel.id == str(attempt_id)
        )
    ...
    async def list_by_task_id(self, task_id: UUID) -> list[TaskAttempt]:
        ...
        result = await self.session.execute(stmt)
        return [TaskAttemptMapper.to_domain(
            model
