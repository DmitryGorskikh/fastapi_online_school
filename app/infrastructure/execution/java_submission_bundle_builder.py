...
class JavaSubmissionBundleBuilder(SubmissionBundleBuilder):
    def build(
        ...
        run_path.write_text(
            self._render_run_script(test_cases), encoding='utf-8'
        )
    ...
    def _render_run_script(self, test_cases: list[TestCase]) -> str:
        ...
        for index, test_case in enumerate(test_cases, start=1):
            ...
            lines.extend([
                f"actual_output_#{index}=$(printf '%s' '{escaped_input}' | java Main | tr -d '\\r')".replace('#', str(index)),  # Noqa E501
                f"expected_output_#{index}=$(printf '%s' '{escaped_expected}' | tr -d '\\r')".replace('#', str(index)),  # Noqa E501
                f"normalized_actual_#{index}=$(printf '%s' \"$actual_output_{index}\" | sed 's/[[:space:]]*$//')",  # Noqa E501
                f"normalized_expected_#{index}=$(printf '%s' \"$expected_output_{index}\" | sed 's/[[:space:]]*$//')",  # Noqa E501
                f"if [ \"$normalized_actual_{index}\" != \"$normalized_expected_{index}\" ]; then",  # Noqa E501
                f"  echo \"Test case {index} failed: expected '$normalized_expected_{index}', got '$normalized_actual_{index}'\" >&2",  # Noqa E501
                '  exit 1',
                'fi',
                '',
            ])
