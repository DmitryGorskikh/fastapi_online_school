...
@pytest.mark.asyncio
async def test_get_code_task_returns_editor_configuration_only(client, seeded_tasks_tree):  # Noqa E501
    response = await client.get(f'/api/code-tasks/{seeded_tasks_tree.code_task_id}')  # Noqa E501
...
@pytest.mark.asyncio
async def test_get_course_structure_returns_tasks_and_code_tasks(client, seeded_tasks_tree):  # Noqa E501
    response = await client.get(f'/api/courses/{seeded_tasks_tree.course_id}/structure')  # Noqa E501
    ...
    assert any(item['id'] == seeded_tasks_tree.task_id for item in section['tasks'])  # Noqa E501
    assert any(item['id'] == seeded_tasks_tree.code_task_id for item in section['code_tasks'])  # Noqa E501
