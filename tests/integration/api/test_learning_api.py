...
async def test_get_question_attempt_returns_question_state(
    ...
    response = await client.get(
        f'/api/learning/questions/{seeded_interactive_tree.question_id}/attempt',  # noqa 501
...
async def test_submit_question_answer_returns_created_attempt(
    ...
    response = await client.post(
        f'/api/learning/questions/{seeded_interactive_tree.question_id}/attempts',  # noqa 501
        headers=student_auth_headers,
        json={'selected_option_ids': [seeded_interactive_tree.correct_option_id]},  # noqa 501
    )
    assert response.status_code == 201
    payload = response.json()
    assert payload['result_status'] == 'correct'
    assert payload['awarded_points'] == 5
    assert payload['selected_option_ids'] == [seeded_interactive_tree.correct_option_id]  # noqa 501
