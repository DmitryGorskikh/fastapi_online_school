...
from app.application.use_cases.auth.refresh_token import (
    RefreshTokenCommand, RefreshTokenUseCase
)
...
class StubTokenService:
    ...
    def create_refresh_token(self, user_id, role: str) -> str:
        return f'refresh-for-{user_id}-{role}'

    def verify_refresh_token(self, token: str):
        return UUID(token.removeprefix('refresh-for-').rsplit('-', 1)[0])
...
async def test_register_user_creates_student_and_commits() -> None:
    ...
    use_case = RegisterUserUseCase(
        uow=uow, password_hasher=StubPasswordHasher()
    )
...
async def test_login_user_returns_access_token() -> None:
    ...
    assert result.refresh_token.startswith('refresh-for-')
...
async def test_refresh_returns_new_token_pair() -> None:
    ...
    token = f'refresh-for-{user.id}-student'
    result = await use_case.execute(RefreshTokenCommand(refresh_token=token))
...
@pytest.mark.asyncio
async def test_refresh_raises_when_user_not_found() -> None:
    uow = FakeUnitOfWork()
    use_case = RefreshTokenUseCase(
        uow=uow,
        token_service=StubTokenService(),
    ...
    token = f'refresh-for-{uuid4()}-student'
    with pytest.raises(InvalidCredentialsError):
        await use_case.execute(RefreshTokenCommand(refresh_token=token))
