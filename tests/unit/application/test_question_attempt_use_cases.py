...
class FakeQuestionAttemptRepository:
    ...
    async def get_by_student_and_question(self, student_id, question_id):
        attempts = [
            ...
            if item.student_id == student_id and item.question_id == question_id  # noqa: E501
        ]
...
@pytest.mark.asyncio
async def test_start_question_attempt_returns_question_state(
) -> None:
    ...
    wrong_option = AnswerOption(
        id=uuid4(), question_id=question.id,
        text='POST', position=1, is_correct=False
    )
    correct_option = AnswerOption(
        id=uuid4(), question_id=question.id,
        text='GET', position=2, is_correct=True
    )
...
@pytest.mark.asyncio
async def test_submit_question_answer_creates_attempt_and_updates_progress(
) -> None:
    uow = FakeInteractiveUnitOfWork()
    student = User(
        id=uuid4(),
    ...
    correct_option = AnswerOption(
        id=uuid4(), question_id=question.id,
        text='GET', position=1, is_correct=True
    )
    wrong_option = AnswerOption(
        id=uuid4(), question_id=question.id,
        text='POST', position=2, is_correct=False
    )
    ...
    progress = await uow.progress.get_by_student_and_course(
...
@pytest.mark.asyncio
async def test_get_question_attempt_result_allows_owner_author(
) -> None:
    uow = FakeInteractiveUnitOfWork()
    author = User(
        id=uuid4(),
    ...
    question = Question(
        id=uuid4(), section_id=section.id, text='Q', position=1
    )
