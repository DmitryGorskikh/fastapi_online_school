...
def test_single_choice_question_requires_exactly_one_correct_option(
    ...
    options = [
        AnswerOption(
            ...
            text='A', position=1, is_correct=True
        ),
        AnswerOption(
            id=uuid4(), question_id=question.id,
            text='B', position=2, is_correct=True
        ),
    ]
    with pytest.raises(InvalidQuestionError):
...
def test_question_resolves_correct_status_and_points(
) -> None:
    ...
    options = [
        AnswerOption(
            id=wrong_option_id, question_id=question.id,
            text='POST', position=1, is_correct=False
        ),
        AnswerOption(
            id=correct_option_id, question_id=question.id,
            text='GET', position=2, is_correct=True
        ),
    ]
    assert question.resolve_result_status(
        [correct_option_id], options
    ) is QuestionResultStatus.CORRECT
    assert question.resolve_awarded_points(
        [correct_option_id], options
    ) == 5
    assert question.resolve_awarded_points(
        [wrong_option_id], options
    ) == 0
