...
def test_progress_applies_correct_attempt_only_once(
) -> None:
    student_id = uuid4()
    question_id = uuid4()
    progress = Progress(
        id=uuid4(), student_id=student_id, course_id=uuid4()
    )
...
def test_progress_syncs_section_and_module_completion(
) -> None:
    question_id = uuid4()
    section = Section(
        id=uuid4(),
